...
for d in ds:
    refs.append("D" + str(d))
templateReferences.extend(refs)
for r in rs:
    refs.append("R" + str(r))
templateReferences.extend(refs)
# print(templateReferences)
...
inputBoard = './SBR001-202.kicad_pcb'
#Output file, original file remains unmodified
outputBoardFile = './SBR001-202_scriptout.kicad_pcb'

templateRefModulo = 100;	#Difference in the reference numbers between hierarchical sheet
templateRefStart = 1200;	#Starting point of numbering in the first hierarchical sheet
move_dx = FromMM(101.5)		#Spacing between clones in x direction
move_dy = FromMM(101.5)		#Spacing between clones in y direction
clonesX = 8			#Number of clones in x direction
clonesY = 1			#Number of clones in y direction


numberOfClones = clonesX * clonesY
print("numberOfClones = " + str(numberOfClones))
...
for templateRef in templateReferences:							#For each module in the template schema
    ...
    if templateModule is not None:
        ...
        for i in range(0, numberOfClones-1):						#Create list of references to be cloned of this module in the template	
            ...
            print("cloneRefNumber = " + str(cloneRefNumber))
